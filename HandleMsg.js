	...
	if (isBlocked) {
        ...
        case prefix+'speed':
            case prefix+'ping':
                ...
                break
				case prefix+'tageveryone':
				if (!isGroupMsg) return aruga.reply(from, 'dasfasf', id)
				if (!isOwnerB) return aruga.reply(from, 'fsafaf', id)
				const taxt = body.slice(13)
				try {
					aruga.tagEveryone(groupId, `pesan : ${taxt}`, true)
				} catch (err) {
					console.log(err)
				}
					break
                case prefix+'setpic':
                    ...
                    if (isMedia) {
                        ...
                        aruga.sendTextWithMentions(from, `Makasih @${serial.replace('@c.us','')} Foto Profilenye..`, true)
                    } else if (quotedMsg && quotedMsg.type == 'image') {
                        const mediaData = await decryptMedia(quotedMsg)
                        const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                        await aruga.setProfilePic(imageBase64)
                        aruga.sendTextWithMentions(from, `Makasih @${serial.replace('@c.us','')} Foto Profilenya 😘`, true)
                    } else {
        ...
        case prefix+'menu':
            ...
            await aruga.sendFileFromUrl(from, php2, 'image.jpg', menuId.help(prefix,pushname,waver), id)
        ...
        case prefix+'menuadmin':
			...
			aruga.reply(from, kodbhs, id)
			break
            case prefix+'kodenuklir':
				const benuk = menuId.kodenuklir()
                await aruga.sendText(from, benuk, id)
                break
        case prefix+'donate':
        case prefix+'donasi':
			const inidonate = menuId.textDonasi()
            await aruga.sendText(from, inidonate, id)
            break
          case prefix+'tod':
	...
	case prefix+'tr':
        ...
        case prefix+'stc':
			if (isMedia && type === 'image') {
				...
				console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
				aruga.reply(from, `Haii ${pushname} jika ingin membuat stiker tanpa dipotong, silahkan gunakan ${prefix}sfull reply/post foto`, id)
            } else if (quotedMsg && quotedMsg.type == 'image') {
                const mediaData = await decryptMedia(quotedMsg, uaOverride)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aruga.sendImageAsSticker(from, imageBase64, { author: "@thoriqazzikra_", pack: "Urbaeexyz", keepScale: false })
				console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
				aruga.reply(from, `Haii ${pushname} jika ingin membuat stiker tanpa dipotong, silahkan gunakan ${prefix}sfull reply/post foto`, id)
			} else {
				aruga.reply(from, mess.error.St, id)
	...
	case prefix+'sgif':
	  aruga.reply(from, mess.wait, id)
           if (isMedia && type === 'video' || mimetype === 'image/gif') {
                try {
                    const mediaData = await decryptMedia(message, uaOverride)
                    await aruga.sendMp4AsSticker(from, mediaData, {crop: true, square: 240, fps: 30, startTime: `00:00:00.0`, endTime : `00:00:07.0`,loop: 0}, {keepScale: false, author: `Urbaeexyz`, pack: `gif siapa nich`}, id)
                } catch (err) {
...
case prefix+'lk21':
	...
	aruga.reply(from, mess.wait, id)
	try {
		const forlk21 = await fetch(`http://api-lk21.herokuapp.com/search?query=${lksearch}`)
		const likejs = await forlk21.json()
		...
		console.log(err)
		aruga.reply(from, `Mungkin film yang anda cari tidak ada diwebsite`, id)
	}
	break
case prefix+'filmkat':
...
aruga.reply(from, mess.wait, id)
try {
	const forkat = await fetch(`https://api-filmapik.herokuapp.com/category?search=${katsearch}`)
	const bekat = await forkat.json()
	...
	await aruga.sendFileFromUrl(from, thp, 'img.jpg', kat, id)
} catch (err) {
	console.log(err)
...
case prefix+'filmapik':
	 ...
	 try {
		 const scpik = await fetch(`https://api-filmapik.herokuapp.com/search?q=${pilem}`)
		 const apikjson = await scpik.json()
		 const { result } = await apikjson
		 let iniapik = `*「 FILM APIK 」*\n`
		 for (let i = 0; i < result.length; i++) {
			 iniapik += `\n─────────────────\n\n*•Title:* ${result[i].title}\n*•Rating:* ${result[i].rating}\n*•Quality:* ${result[i].quality}\n*•Episode:* ${result[i].episode}\n*•Id:* ${result[i].movieId}\n*•Views:* ${result[i].detail.views}\n*•Genre:* ${result[i].detail.genre}\n*•Duration:* ${result[i].detail.duration}\n*•Director:* ${result[i].detail.director}\n*•Actors:* ${result[i].detail.actors}\n*•Country:* ${result[i].detail.country}\n*•Released Year:* ${result[i].detail.release}\n*•Description:* ${result[i].detail.description}\n`
		 }
		 const portrait = result[0].thumbnailPotrait
		 const landscape = result[0].detail.thumbnailLandscape
		 if (landscape == '' || landscape == undefined) {
			 var pipa = portrait
		 } else {
			 var pipa = landscape
		 }
		 await aruga.sendFileFromUrl(from, pipa, 'img.jpg', iniapik, id)
	 } catch (err) {
		 console.log(err)
...
case prefix+'ytsearch':
    ...
    break
        ...
        case prefix+'memes':
            aruga.reply(from, mess.wait, id)
            aruga.sendFileFromUrl(from, `https://lindow-api.herokuapp.com/api/random/meme?apikey=LindowGanteng`, 'img.jpg', '', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
    ...
    case prefix+'kusonime':
		...
		await aruga.sendImageAsSticker(from, `http://lolhuman.herokuapp.com/api/ephoto1/goldplaybutton?apikey=dd42c2d20db7c924ccf66f5f&text=${yuza}`, { author: "@thoriqazzikra_", pack: "Urbaeexyz", keepScale: true })
		.catch((err) => {
			aruga.reply(from, 'Error om', id)
			console.log(err)
		})
		...
		await aruga.sendImageAsSticker(from, `http://lolhuman.herokuapp.com/api/ephoto1/silverplaybutton?apikey=dd42c2d20db7c924ccf66f5f&text=${silsc}`, { author: "@thoriqazzikra_", pack: "Urbaeexyz", keepScale: true })
		.catch((err) => {
			aruga.reply(from, 'Error om', id)
			console.log(err)
		})
...
console.log(err)
		...
		case prefix+'tiktok2':
			 ...
			 aruga.reply(from, mess.wait, id)
			 axios.get(`https://api.vhtear.com/tiktok_no_wm?link=${bodynya}&apikey=${vhtearkey}`)
			 .then(async(res) => {
				 const saveiti = await fetch(res.data.result.video);
				 const buffas = await saveiti.buffer();
				 await sleep(1000)
				 fs.writeFile('./media/tktk.mp4', buffas)
				 aruga.sendFile(from, './media/tktk.mp4', 'tiktok.mp4', `nehh om`, id)
				 .catch(() => {
					 aruga.reply(from, 'Error', id)
				 })
			 })
            ...
            case prefix+'tiktok':
				...
				aruga.reply(from, mess.wait, id)
				axios.get(`https://fzn-gaz.herokuapp.com/api/tiktok?url=${linktik}`)
				.then(async(res) => {
					await aruga.sendFileFromUrl(from, res.data.result, 'nowm.mp4', `*from:* ${res.data.nama}\n*caption:* ${res.data.caption}`, id)
					.catch(() => {
			...
			axios.get(`https://lindow-api.herokuapp.com/api/ytmp4?link=${linkmp4}`)
			.then(async(res) => {
				aruga.sendFileFromUrl(from, res.data.result.thumb, 'thumb.jpg', `「 *YOUTUBE MP4* 」\n\n*Title:* ${res.data.result.title}\n*Filesize:* ${res.data.result.filesize}\n*Resolution:* ${res.data.result.resolution}\n\n*_${mess.wait}_*`, id)
				const beas = await axios.get(`http://docs-jojo.herokuapp.com/api/shorturl-at?url=${res.data.result.result}`)
				const beas2 = beas.data.result
				if (Number(res.data.result.filesize.split(' MB')[0]) >= 30) return aruga.reply(from, `Maaf size terlalu besar, silahkan download menggunakan link\n\nLink: ${beas2}`, id)
				if(!isPrem) return aruga.reply(from, `Karena anda bukan user Premium, silahkan download menggunakan Link\n\nLink: ${beas2}`, id)
				await aruga.sendFileFromUrl(from, res.data.result.result, 'vid.mp4', '', id)
				.catch(() => {
           ...
           if (args.length == 0) return aruga.reply(from, `Format pesan salah!\nContoh : ${prefix}findstiker gore|4`, id)
              ...
              try {
				  ...
				  for (let i = 0; i < jumstik; i++) {
					  await aruga.sendStickerfromUrl(from, beres.result.data[i], { author: `@thoriqazzikra_`, pack: `Urbaeexyz`, keepScale: true })
				  }
			  } catch (err) {
...
*Text : ${swrt}*`
		...
		if (isMedia && isImage || isQuotedImage) {
			...
			.then(async (buffer) => {
				...
				await aruga.sendImageAsSticker(from, `${sender.id}_trigger.jpg`, { author: '@thoriqazzikra_', pack: 'Urbaeexyz' })
		...
		aruga.sendImageAsSticker(from, tahtuy, { author: "@thoriqazzikra_", pack: "Urbaeexyz", keepScale: true })
		.catch((err) => {
			aruga.reply(from, 'Error', id)
	})
	.catch(() => {
	...
	case prefix+'bot':
		...
		axios.get(`https://lindow-api.herokuapp.com/api/simi?text=${arbu}&lang=id`).then(res => {
		console.log(color(`${arbu}`, 'green'))
		const segey = `${res.data.response}`
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us','')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
                ...
                case prefix+'ttp':
                     axios.get(`https://st4rz.herokuapp.com/api/ttp?kata=${body.slice(5)}`)
                        .then(async(res) => {
						 aruga.sendImageAsSticker(from, res.data.result, { author: "@thoriqazzikra_", pack: "Urbaeexyz", keepScale: true })
						 .catch((err) => {
							 console.log(err)
						 })
                     })
