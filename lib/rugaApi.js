...
const play = async (lagu) => new Promise((resolve, reject) => {
	axios.get(`https://api.xteam.xyz/dl/ytmp3?url=${lagu}&APIKEY=db0e06bd9f096399`)
	.then((res) => {
		resolve(res.data)
	})
	.catch((err) => {
...
const happymod = async (query) => new Promise((resolve, reject) => {
    axios.get(`https://tobz-api.herokuapp.com/api/happymod?q=${query}&apikey=BotWeA`)
    .then((res) => {
        resolve(res.data)
    })
    .catch((err) => {
...
const youwatch = async (url) => new Promise((resolve, reject) => {
	...
	axios.get(`https://tobz-api.herokuapp.com/api/youwatch?q=${url}&apikey=BotWeA`)
	.then((result) => resolve(result))
	.catch((err) => reject(err))
})
