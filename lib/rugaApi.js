...
const ymp3 = async (url) => new Promise((resolve, reject) => {
	axios.get(`https://lindow-api.herokuapp.com/api/ytmp3?link=${url}&apikey=${lindowapi}`)
    .then((res) => {
		resolve(res.data)
    })
    .catch((err) =>{
...
const ymp4 = async (url) => new Promise((resolve, reject) => {
    axios.get(`http://fahmiapi.herokuapp.com/yt/mp4?url=${url}&apikey=freeTrial2k21`)
    .then((res) => {
		resolve(res.data.result)
    })
    .catch((err) =>{
...
const githubpict = async (url) => new Promise((resolve, reject) => {
	axios.get(`https://videfikri.com/api/github/?username=${url}`)
	.then((res) => {
        if (res.data.error) resolve('https://c4.wallpaperflare.com/wallpaper/976/117/318/anime-girls-404-not-found-glowing-eyes-girls-frontline-wallpaper-preview.jpg')
        resolve(`${res.data.result.profile_pic}`)
    })
    .catch((err) => {
        reject(err)
    })
